...
const loginGoogleRequest = (idToken, role) => async (dispatch) => {
  dispatch({ type: types.LOGIN_GOOGLE_REQUEST, payload: null });
  try {
    const res = await api.post("/auth/login/google", { idToken, role });
    const name = res.data.data.user.name;
    toast.success(`Welcome ${name}`);
...
const register = (role, name, email, password) => async (dispatch) => {
  dispatch({ type: types.REGISTER_REQUEST, payload: null });
  try {
    const res = await api.post("/users", { role, name, email, password });
    console.log(res);
